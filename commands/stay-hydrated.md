...
argument-hint: "[setup <ml_dia> <horas> [ml_copo]] | drank | postpone | status"
...
- starts with `setup` → run `${CLAUDE_PLUGIN_ROOT}/scripts/hydration-setup.sh` with the remaining args (ml_per_day cc_hours [ml_per_glass]). If the user gave no numbers, ask them: how many ml/day, how many hours/day do you use Claude Code, and ml per glass (default 250).
