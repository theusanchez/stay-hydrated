...
jq -n \
  ...
  '{daily_ml:$daily_ml, cc_hours:$cc_hours, per_drink_ml:$per_drink_ml, start_hour:$start_hour, grace_min:$grace_min, max_postpones:$max_postpones, postpone_min:$postpone_min, interval_min:$interval_min, num_drinks:$num_drinks}' \
  > "$CONFIG_FILE"
...
echo "   Início do dia: ${START_HOUR}h · 1 copo a cada ${INTERVAL_MIN} min"
...
if day_active; then
  ...
  echo "   Aguardando ${START_HOUR}h para começar a contar."
fi
